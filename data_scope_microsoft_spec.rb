...
describe DataScope do
  it "should login" do
    VCR.use_cassette "ms_login", record: :new_episodes do
  ...
  it "should extract with ISIN" do
    VCR.use_cassette "ms_extract_with_isin", record: :new_episodes do
      api = DataScope::Api.new
      req = api.extract_with_isin "US5949181045"
  ...
  it "should get result" do
    VCR.use_cassette "ms_get_result", record: :new_episodes do
      api = DataScope::Api.new
      req = api.extract_with_isin "US5949181045"
      req.get_result
      ...
      expect(req.result["Contents"]).to eq(
                                          [{"Close Price"=>126.18,
                                            "Contributor Code Description"=>"NASDAQ Stock Exchange Global Select Market",
                                            "Currency Code Description"=>"U.S. Dollar",
                                            "Dividend Yield"=>1.441215634,
                                            "Identifier"=>"US5949181045",
                                            "IdentifierType"=>"Isin",
                                            "Main Index"=>"NASDAQ Composite Index",
                                            "Market Capitalization"=>966894365145.6,
                                            "Market Capitalization - Local Currency"=>"USD",
                                            "Percent Change - Close Price - 1 Day"=>-1.167071356,
                                            "Universal Close Price Date"=>"2019-05-23"}]
                                        )
    end
  end

  it "should extract with location", record: :new_episodes do
    VCR.use_cassette "ms_extract_with_location" do
      api = DataScope::Api.new
